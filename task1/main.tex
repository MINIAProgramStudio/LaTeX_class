...
{
\begin{align*}
    \Sigma_{1,\theta} &= \left\{a \in \mathbb{C} \backslash ]-\infty, -1]: \mathfrak{R} \left(\int_{[1,a]}e^{i\theta}\sqrt{p_a(z)}dz\right)=0\right\};\\
    \Sigma_{-1,\theta} &= \left\{a \in \mathbb{C} \backslash [1, +\infty[ : \mathfrak{R} \left(\int_{[-1,a]}e^{i\theta}\sqrt{p_a(z)}dz\right)=0\right\};\\
    \Sigma_\theta &= \left\{a \in \mathbb{C} \backslash [-1, 1] : \mathfrak{R} \left(\int_{[-1,1]}e^{i\theta}\sqrt{p_a(z)}dz\right)=0\right\};
\end{align*}
}
...
\begin{align*}
    0 &= \int\nolimits_0^1\sqrt{t(1-t)}\mathfrak{I}\left(e^{i\theta}(a-1)^2\sqrt{t(a-1)+2}\right)dt;\\
    0 &= \mathfrak{R}\left((a-1)^3 g'(a)\right) = \int\nolimits_0^1 t\sqrt{t(1-t)}\mathfrak{I}\left(\frac{e^{i\theta}(a-1)^3}{2\sqrt{t(a-1)+2}}\right)dt.
\end{align*}

За неперервністю функцій всередині цих інтегралів на відрізку $[0,1]$,
існують $t_1, t_2 \in [0,1]$ такі що
$$\mathfrak{I}\left(e^{i\theta}(a-1)^2\sqrt{t_1(a-1)+2}\right) = \mathfrak{I} \left(\frac{e^{i\theta}(a-1)^3}{2\sqrt{t_2(a-1)+2}}\right) = 0;$$
а потім
$$e^{2i\theta}(a-1)^4(t_1(a-1)+2) > 0, \left(\frac{e^{2i\theta}(a-1)}{t_2(a-1)+2}\right) > 0.$$
яка не може виконуватись, оскільки, якщо $\mathfrak{I} a > 0$, то
\begin{multline*}
