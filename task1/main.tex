...
\pretocmd{\endmyproof}{\hfill$\square$}{}{}
...
Таким чином, будь-який промінь з $\Sigma_{\pm 1, \theta}$ повинен розходитись на $\infty$.
Випадок $\theta = 0$ є простішим.

Якщо $a \rightarrow \infty$, тоді $|f_{1, \theta}(a)| \rightarrow \infty$; оскільки $\Re f_{1_\theta}(a) = 0$, 
отримуємо $|\Im f_{1, \theta}(a)| \rightarrow +\infty$.
Звідси випливає, що
$$\arg(f(a)) \sim \arg\left(\frac{4}{15}e^{i \theta}a^{5/2}\right) \rightarrow \frac{\pi}{2} + k\pi, k \in \mathbb{Z} as a \rightarrow \infty$$
...
Якщо $\Sigma_{1, \theta}$ містить регулярну точку $z_0$ (наприклад, $\Im z_0 > 0$),
яка не належить дугам $\Sigma_{1, \theta}$, що виходять з точки $a=1$.
...
або $z_1 = y$, для деяких $y>1$.
Легко перевірити, що в обох випадках, для будь-якого вибору аргументу,
$$\Re \int_1^{z_1}\left(e^{i\theta}\sqrt{p_{z_1}(t)}dt\right) \neq 0;$$
і отримуємо протиріччя. Таким чином,
$\Sigma_{1, \theta}$ утворюється лише двома кривими, що проходять через $a = 1$.
Таку саму ідею дає структура $\Sigma_{-1, \theta};$
навіть більше, з співвідношення
\begin{equation}
    \Re f_{\pm 1, \theta}(a) = 0 \Longleftrightarrow \Re f_{\pm 1, \frac{\pi}{2} - \theta}(-\overline{a}) = 0
    \label{equation_2}
\end{equation}
доступних для довільного $\theta \in [\pi/4, \pi/2[$,
можна легко побачити що $\Sigma_{-1, \frac{\pi}{2}-\theta}$ і $\Sigma_{1, \theta}$ симетричні відносно уявної осі (\ref{equation_2}).
